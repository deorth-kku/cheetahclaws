...
- July 11, 2026: **Terminal tab title tracks the live task, plus a cross-turn fix for the Anthropic prompt cache.** The terminal window/tab title now reflects what CheetahClaws is doing — a pulsing glyph + your current prompt while working (`✶ ✳ ✻ CheetahClaws — <task>`), a static badge when idle — emitted as an OSC 0 sequence and auto-disabled on non-TTYs/pipes/CI. It works out of the box in iTerm2/Terminal.app; **VS Code/Cursor/Windsurf hide program-set titles by default**, so first launch configures `terminal.integrated.tabs.title` once (backup + re-parse validation, never overwrites a value you set), re-runnable with `/terminal-setup`. Toggle via `terminal_title`. Separately, the Anthropic `cache_control` breakpoint moved onto the stable system span *before* the volatile per-run environment block, so a `git status` change no longer invalidates the large static base prompt's cache across turns. [Details](docs/news.md)
