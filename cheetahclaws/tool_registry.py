...
def get_tool_schemas(
    ...
    ``disabled_tools`` matching is case-insensitive and ignores surrounding
    whitespace, so a config value like ``["bash", " WebFetch "]`` filters
    those tools out. The model therefore never receives the declarations
    ...
    return [
        ...
        if tool.name.strip().lower() not in disabled
        and _profile_allows(tool, active_profile)
    ]
...
def _effective_output_cap(config: Dict[str, Any], max_output: int) -> int:
    ...
    # Explicit user config wins over the static default (tri-state: None
    # means "unset", so the caller's max_output stands).
    conf_max_output = (config or {}).get("max_tool_output")
    if conf_max_output:
        try:
            max_output = int(conf_max_output)
