...
def _maybe_redirect_to_summarize(text: str, file_path: str,
                                    config: dict,
                                    total_chars: int | None = None) -> str | None:
    ...
    ``total_chars`` overrides the size estimate when the caller only has
    a budget-capped PREFIX of a larger file: pass the file's estimated
    true character count so the threshold is judged on the file's real
    size, not the truncated slice. The preview still comes from `text`.
    """
    if not text:
        return None
    ...
    # No artificial ceiling here (an earlier version capped this at
    # 30000). The cap redirected far too aggressively on large-context
    # models and, combined with the related max_tokens under-cap, made
    # replies truncate mid-sentence while context was still available.
    # Trust the declared/registry limit instead — models whose registry
    # entry is small (e.g. qwen2.5-72b → 32768) still get an early
    # redirect, which is the case that actually overflows.
    ...
    chars = total_chars if total_chars is not None else len(text)
    if _is_cjk_heavy(text):
        estimated_tokens = chars
    else:
        estimated_tokens = int(chars * _TOKENS_PER_CHAR)
