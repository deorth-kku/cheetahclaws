...
def _detect_encoding(data: bytes) -> str: 
    ...
    if result['confidence'] < 0.1:
        raise OSError(f"File does not appear to be text (chardet result {result})")
...
def _read(
    ...
    try:
        ...
        # Detect the file's text encoding once from a bounded sample so
        # non-UTF-8 sources (GBK, Latin-1, ...) still render. Binary files
        # (NUL bytes) raise here and are reported via the except clause.
        if file_size == 0:
            encoding = "utf-8"
        else:
            with p.open("rb") as _probe:
                encoding = _detect_encoding(_probe.read(8192))
        with p.open("rb") as handle:
            while True:
                ...
                # incomplete tail preserves valid text and the byte ceiling.
                text = raw.decode(
                    encoding, errors="ignore" if capture_hit or scan_hit else "replace",
                )
                line_no += 1
